...
const VerifyEmailPage = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    setLoading(true);
    try {
      const res = await callVerifyEmail(email as string, verificationCode);
      if (res && res.status === 200) {
        message.success("Xác thực email thành công! Vui lòng đăng nhập.");
        navigate("/login");
      } else {
        message.error(
          "Mã xác thực không đúng hoặc hết thời hạn! Vui lòng thử lại."
        );
      }
    } catch (error) {
  ...
  const handleResendToken = async () => {
    ...
    if (resendAttempts >= MAX_RESEND_ATTEMPTS) {
      message.error(
        "Bạn đã vượt quá số lần gửi lại mã cho phép. Vui lòng liên hệ hỗ trợ!"
      );
      return;
    }

    try {
      const res = await callResendToken(email as string);
      if (res && res.status === 200) {
      ...
      <div className="min-h-screen bg-gray-100 flex items-center justify-center py-2 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-md">
          <div className="text-center">
            <h2 className="mt-6 text-3xl font-extrabold text-gray-900">Lỗi</h2>
