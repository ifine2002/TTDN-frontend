...
import { IBookSearch, IUser } from "@/types/backend";

const { TabPane } = Tabs;
const { Search } = Input;

type TActiveTab = "books" | "users";
...
const SearchPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<TActiveTab>("books");
  const [bookData, setBookData] = useState<IBookSearch[]>([]);
  const [userData, setUserData] = useState<IUser[]>([]);
  const [bookPagination, setBookPagination] = useState<IPagination>({
    current: 0,
    pageSize: 8,
    total: 0,
  });
  const [userPagination, setUserPagination] = useState<IPagination>({
    current: 0,
    pageSize: 8,
    total: 0,
  });
  ...
  useEffect(() => {
    ...
    setSearchText(query);
    const validTab: TActiveTab = tab === "users" ? "users" : "books";
  ...
  const handleTabChange = (key: string) => {
    ...
    const validKey: TActiveTab = key === "users" ? "users" : "books";
  ...
  const fetchBooks = async (query: string, page: number) => {
    ...
    try {
      setLoading(true);
      const queryParams = `page=${page}&size=${
                ...
                <Empty
                  description={
                    searchText
                      ...
                      : "Vui lòng nhập từ khóa và nhấn tìm kiếm"
                  }
                />
              )}
            </Spin>
