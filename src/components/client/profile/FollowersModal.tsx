...
interface IProps {
  ...
  setActiveModalTab: (v: string) => void;
  userData: IUser;
  listFollowing: IUser[];
  currentUser: any;
}
...
const FollowersModal = ({
  ...
  const isUserInFollowingList = (userId: number) => {
  ...
  const renderFollowButton = (user: any) => {
    ...
    return (
      <Button
        ...
        className={`${
          isUserInFollowingList(user.id)
            ? "bg-gray-500 hover:!bg-gray-600"
            : "bg-blue-500 hover:!bg-blue-600"
        } w-20 !text-white hover:!text-white`}
        style={{ border: "none" }}
      >
  ...
  return (
    ...
    >
      ...
      >
        <TabPane
          tab={`Follower ${userData.follower?.length || 0}`}
          key="followers"
        >
          {userData.follower && userData.follower.length > 0 ? (
            <List
              ...
              renderItem={(user, index) => (
                <>
                  <List.Item actions={[renderFollowButton(user)]}>
                    <List.Item.Meta
                      avatar={
                        <Avatar
                          src={user.image}
                  ...
                  {index < userData.follower!.length - 1 && (
        ...
        <TabPane
          tab={`Đã follow ${userData.following?.length || 0}`}
          key="following"
        >
            ...
            <List
              ...
              renderItem={(user, index) => (
                <>
                  ...
                  {index < userData.following!.length - 1 && (
                    <Divider className="my-2" />
                  )}
                </>
              )}
